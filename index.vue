  ...
  <div :class="['workspace', { 'budget-flow-workspace': prototypeVariant === 'flow-template' }]">
            ...
            <article id="basic" class="section-card">
              ...
              <el-form :model="form" label-position="top" :disabled="!isEditing">
                ...
                <el-row :gutter="20" class="business-affiliation-row">
                  ...
                  <el-col :span="6"><el-form-item label="预算有效期（天）" required><el-input-number v-model="form.validDays" :min="1" /><div class="field-tip"><InfoFilled />提示：需要在预算审批通过后{{ form.validDays }}天内完成销售、采购订单的创建</div></el-form-item></el-col>
                </el-row>
                ...
                <el-row :gutter="20">
                  <el-col :span="8"><el-form-item label="供应商" required><el-select v-model="form.supplier"><el-option label="成都星海科技有限公司" value="成都星海科技有限公司" /></el-select></el-form-item></el-col>
            ...
            <article id="plan" class="section-card">
              ...
              <div class="split-panel">
                <div class="plan-form"><h3>预算计划</h3>
                  <el-form :model="form" label-position="top" :disabled="!isEditing">
                    <el-row :gutter="16">
                      <el-col :span="12"><el-form-item label="预计到货日期" required><el-date-picker v-model="form.arrival" type="date" value-format="YYYY-MM-DD" /></el-form-item></el-col>
                      <el-col :span="12"><el-form-item label="预计付款日期" required><el-date-picker v-model="form.payment" type="date" value-format="YYYY-MM-DD" /></el-form-item></el-col>
                      <el-col :span="12"><el-form-item label="采购账期（天）" required><el-input-number v-model="form.purchaseDays" :min="0" /></el-form-item></el-col>
                      <el-col :span="12"><el-form-item label="最长销售周期（天）" required><el-input-number v-model="form.saleDays" :min="1" /></el-form-item></el-col>
                      ...
                      <el-col v-if="form.type === '产品导向分销'" :span="12"><el-form-item label="项目单毛利回补总额"><el-input-number v-model="form.dynamic.profitBackfill" :min="0" /></el-form-item></el-col>
    ...
    <Teleport to="body"><section v-if="activeContractEditPage" :class="['document-page', 'contract-edit-page', `mode-${contractDraft.contractMode}`, `type-${contractDraft.type}`]">
      ...
      <div ref="contractScrollArea" class="document-scroll" @scroll="updateContractCurrentModule">
        ...
        <section id="contract-content-section" data-contract-module="contract-content-section" class="contract-block contract-info-block"><h3><b>04</b>合同内容</h3><h4 class="contract-content-subtitle">合同信息</h4><el-form label-position="top"><el-row :gutter="16">
          ...
          <el-col :span="24" class="contract-field-group-title">{{ contractDraft.type === 'purchase' ? '合同金额与付款' : '合同金额与回款' }}</el-col><el-col v-if="contractDraft.contractMode !== 'framework'" :span="8"><el-form-item label="合同税率情况"><el-select v-model="contractDraft.taxRate"><el-option v-for="rate in 20" :key="rate" :label="`${rate}%`" :value="`${rate}%`" /></el-select></el-form-item></el-col>
          ...
          <template v-if="contractDraft.contractMode !== 'framework'"><el-col :span="8" class="primary-amount-field"><el-form-item label="合同总金额（含税）"><el-input model-value="¥380,000.00" disabled /></el-form-item></el-col><el-col :span="8" class="primary-amount-field"><el-form-item label="合同总金额（不含税）"><el-input model-value="¥336,283.19" disabled /></el-form-item></el-col><el-col :span="8" class="primary-amount-field"><el-form-item label="合同税额"><el-input model-value="¥43,716.81" disabled /></el-form-item></el-col><el-col :span="8"><el-form-item :label="contractDraft.type === 'purchase' ? '货款结算' : '回款约定'"><el-input v-model="contractDraft.settlement" /></el-form-item></el-col></template>
    ...
    <Teleport to="body"><section v-if="activeOrderPage" class="document-page order-edit-page">
      ...
      <div ref="orderScrollArea" class="document-scroll order-document-scroll" @scroll="updateOrderCurrentModule">
        ...
        <article id="order-attachments" class="section-card"><SectionTitle number="05" title="附件与说明" /><el-row :gutter="20"><el-col :span="12"><el-form label-position="top"><el-form-item label="备注"><el-input v-model="orderDraft.remark" type="textarea" :rows="4" maxlength="500" show-word-limit /></el-form-item></el-form></el-col><el-col :span="12"><el-upload drag action="#" :auto-upload="false"><Upload /><div>拖拽文件到此处，或点击上传附件</div><small>最多上传5个文件</small></el-upload></el-col></el-row></article>
...
const contractDetailModules = computed(() => {
  ...
  return [{ order: '01', label: '基础信息' }, { order: '02', label: '关联单据信息' }, { order: '03', label: purchase ? '采购清单' : '销售清单' }, { order: '04', label: '合同内容' }, { order: '05', label: activeContractPage.value?.data?.contractMode === 'framework' ? '订单证明文件' : '合同文件' }, { order: '06', label: '签收文件与归档信息' }, ...(purchase ? [{ order: '07', label: '后续任务' }] : []), { order: purchase ? '08' : '07', label: '审批信息' }]
})
...
function switchView(view) { activeView.value = view; activeTab.value = view; if (!tabs.value.some(x => x.key === view)) tabs.value.push({ key: view, title: view === 'approval' ? '我的审批' : view === 'contract' ? '合同管理' : '分销产品预算', closable: view !== 'list' }) }
function activateTab(key) { activeTab.value = key; const tab = tabs.value.find(x => x.key === key); if (['list', 'approval', 'contract'].includes(key)) activeView.value = key; else if (tab?.supplierPage) activeView.value = 'contract'; else if (tab?.orderPageMode) { activeView.value = 'contract'; orderDraft.budget = tab.data?.budget || budgets.value[1]; orderDraft.type = tab.orderType; orderDraft.typeLabel = tab.orderType === 'purchase' ? '采购订单' : '销售订单' } else if (tab?.contractPageMode) { activeView.value = 'contract'; contractPageMode.value = tab.contractPageMode; if (tab.contractPageMode !== 'detail') { prepareContractDraft(tab.data?.budget || budgets.value[1], tab.contractType); contractDraft.contractCode = tab.data?.code || '' } } else { activeView.value = 'list'; mode.value = tab.mode; Object.assign(documentData, tab.data) } }
...
/* Contract content follows the same decision order in create, detail and approval. */
.contract-info-block > .contract-content-subtitle,
.contract-detail-info > .contract-content-subtitle:first-of-type {
  font-size: 0;
}
.contract-info-block > .contract-content-subtitle::after,
.contract-detail-info > .contract-content-subtitle:first-of-type::after {
  content: '交易与金额';
  color: #303b47;
  font-size: 14px;
  font-weight: 600;
